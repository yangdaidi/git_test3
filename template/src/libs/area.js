 /* ----------------------------------------------------------------------------
 * @description   省市区(县)三级联动数据类，
 * @version       1.0
 * @author        hzyangyang2015
 * --------------------------------------------------------------------------*/
 // 新增空省（交互需求，地址选择框中不要默认任何地区）
let areaMap = {
         s:{'北京市':1,'天津市':1,'上海市':1,'重庆市':1},
         p:['省','北京市','天津市','上海市','重庆市','河北省','山西省','辽宁省','吉林省','黑龙江省','江苏省','浙江省','安徽省','福建省','江西省','山东省','河南省','湖北省','湖南省','广东省','海南省','四川省','贵州省','云南省','陕西省','甘肃省','青海省','西藏自治区','内蒙古自治区','广西壮族自治区','宁夏回族自治区','新疆维吾尔自治区'],
         c:{'省':['市'],
         ...
         a:{'省-市':['区'],
            ...
            '江苏省-苏州市':['姑苏区','园区','新区','吴中区','相城区','吴江区','常熟市','张家港市','昆山市','太仓市'],
...
class areaClass {
     ...
     * @return {Array}  城市列表
     */
    getCityList(_province){
        return this._dmap.c[_province]||[];
    };
    /**
     * 取区域列表
     * @protected
     ...
     * @return {Array}  城市列表
     */
    getAreaList(_province,_city){
        return this._dmap.a[_province+'-'+_city]||[];
    };
    /**
     * 判断给定省份是否有三级目录
     *
     ...
     */
    hasArea(_province){
        return !this._dmap.s[_province];
    };
}

let area = new areaClass();
export default area;